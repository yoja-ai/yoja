...
def oauth2cb(event, context):
    ...
    try:
        ...
        # https://docs.aws.amazon.com/amazondynamodb/latest/APIReference/API_PutItem.html: replaces old item with new.  So overrides folder_id attribute of the item during login
        # boto3.client('dynamodb').put_item(TableName=os.environ['USERS_TABLE'],
        #                                     Item={'email': {'S': email},
        #                                             'refresh_token': {'S': refresh_token},
        #                                             'access_token': {'S': access_token},
        #                                             'id_token': {'S': id_token},
        #                                             'created': {'N': str(created)},
        #                                             'expires_in': {'N': str(expires_in)}})
    except Exception as ex:
