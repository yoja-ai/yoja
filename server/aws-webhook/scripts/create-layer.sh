...
#/bin/rm -f /tmp/yoja-webhook-layer.zip
...
(cd layer-build; docker run -v "$PWD":/var/task "public.ecr.aws/sam/build-python3.11:latest" /bin/sh -c "pip install -r requirements.txt -t python/lib/python3.11/site-packages/; exit")
sudo /bin/rm -rf layer-build/python/lib/python3.11/site-packages/googleapiclient/discovery_cache/documents
#(cd layer-build; zip -r /tmp/yoja-webhook-layer.zip .)
